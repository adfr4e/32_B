...
typedef struct
{
	...
	DataBits_TypeDef DataBits; // 数据位位数，可取值 UART_DATA_5BIT、UART_DATA_6BIT  UART_DATA_7BIT、UART_DATA_8BIT

	Parity_TypeDef Parity; // 奇偶校验位，可取值 UART_PARITY_NONE、UART_PARITY_ODD、UART_PARITY_EVEN、UART_PARITY_ONE、UART_PARITY_ZERO

	StopBits_TypeDef StopBits; // 停止位位数，可取值 UART_STOP_1BIT、UART_STOP_2BIT

	uint8_t RXIEn; // 接收数据有效中断 ENABLE or DISABLE
} UART_InitTypeDef;
...
uint8_t UART_IsTXEmpty(UART_TypeDef *UARTx);					   // 串口发送寄存器是否为空
uint8_t UART_IsRXValid(UART_TypeDef *UARTx);					   // 串口接收寄存器是否有数据
